...
use itlo\cms\assets\CmsAsset;
use itlo\cms\base\Module;
use itlo\cms\helpers\ComposerHelper;
use itlo\cms\helpers\FileHelper;
use itlo\cms\models\CmsExtension;
...
use itlo\cms\modules\admin\widgets\formInputs\OneImage;
use itlo\cms\rbac\CmsManager;
use itlo\cms\relatedProperties\PropertyType;
use itlo\cms\relatedProperties\propertyTypes\PropertyTypeBool;
use itlo\cms\relatedProperties\propertyTypes\PropertyTypeElement;
use itlo\cms\relatedProperties\propertyTypes\PropertyTypeFile;
use itlo\cms\relatedProperties\propertyTypes\PropertyTypeList;
use itlo\cms\relatedProperties\propertyTypes\PropertyTypeListMulti;
use itlo\cms\relatedProperties\propertyTypes\PropertyTypeNumber;
use itlo\cms\relatedProperties\propertyTypes\PropertyTypeRadioList;
use itlo\cms\relatedProperties\propertyTypes\PropertyTypeSelect;
use itlo\cms\relatedProperties\propertyTypes\PropertyTypeSelectMulti;
use itlo\cms\relatedProperties\propertyTypes\PropertyTypeStorageFile;
use itlo\cms\relatedProperties\propertyTypes\PropertyTypeString;
use itlo\cms\relatedProperties\propertyTypes\PropertyTypeText;
use itlo\cms\relatedProperties\propertyTypes\PropertyTypeTextarea;
use itlo\cms\relatedProperties\propertyTypes\PropertyTypeTextInput;
use itlo\cms\relatedProperties\propertyTypes\PropertyTypeTree;
use itlo\cms\relatedProperties\userPropertyTypes\UserPropertyTypeColor;
use itlo\cms\relatedProperties\userPropertyTypes\UserPropertyTypeComboText;
use itlo\cms\relatedProperties\userPropertyTypes\UserPropertyTypeDate;
use itlo\cms\relatedProperties\userPropertyTypes\UserPropertyTypeSelectFile;
 ...
 *
 * @property \itlo\cms\modules\admin\Module $moduleAdmin
 * @property \itlo\cms\Module               $moduleCms
 ...
 */
class Cms extends \itlo\cms\base\Component
{
    /**
     ...
     */
    public $adminEmail = 'admin@itlo.ru';
    /**
     * @var string
     */
    public $appName;
                ...
                'fields' => [
                    'languageCode' => [
                        ...
                        'items' => \yii\helpers\ArrayHelper::map(
                            \itlo\cms\models\CmsLang::find()->active()->all(),
                            'code',
                            'name'
                        ),
                    ],
                ...
                'fields' => [
                    'access' => [
                        ...
                        'content' => function (HtmlBlock $htmlBlock) {
                            ...
                            $content .= \itlo\cms\modules\admin\widgets\BlockTitleWidget::widget([
                                'content' => "Файлы",
                            ]);
                            $content .= \itlo\cms\rbac\widgets\adminPermissionForRoles\AdminPermissionForRolesWidget::widget([
                                'permissionName' => \itlo\cms\rbac\CmsManager::PERMISSION_ELFINDER_USER_FILES,
                                'label'          => 'Доступ к личным файлам',
                            ]);
                            $content .= \itlo\cms\rbac\widgets\adminPermissionForRoles\AdminPermissionForRolesWidget::widget([
                                'permissionName' => \itlo\cms\rbac\CmsManager::PERMISSION_ELFINDER_COMMON_PUBLIC_FILES,
                                'label'          => 'Доступ к общим файлам',
                            ]);
                            $content .= \itlo\cms\rbac\widgets\adminPermissionForRoles\AdminPermissionForRolesWidget::widget([
                                'permissionName' => \itlo\cms\rbac\CmsManager::PERMISSION_ELFINDER_ADDITIONAL_FILES,
    ...
    /**
     * @return null|\itlo\cms\modules\admin\Module
     */
    ...
    public function generateTmpConfig()
    {
        ...
        $configs = array_unique(array_merge(
            [
                \Yii::getAlias('@itlo/cms/config/main.php'),
            ], $configs
        ));
    ...
    public function getRelatedHandlersDataForSelect()
    {
        ...
        return [
            \Yii::t('itlo/cms', 'Base types')   => $baseTypes,
            \Yii::t('itlo/cms', 'Custom types') => $userTypes,
        ];
    }
    /**
    ...
    {
        return (string)ArrayHelper::getValue(\Yii::$app->extensions, 'itlo/cms.version');
    }
